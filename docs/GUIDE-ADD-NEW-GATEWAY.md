# [GUIDE] Adicionando um novo gateway de pagamento
...
- i18n: strings do cliente e **rótulos/descrições de settings** via `__()`/`esc_html__()` com text domain `paycrypto-me-for-woocommerce`. **Erros, warnings, logs, feedback de botão de diagnóstico e notas de pedido do admin ficam em inglês literal**, sem `__()` — a regra completa e o porquê estão em [GUIDE-TRANSLATION.md](./GUIDE-TRANSLATION.md) → "O que entra (e o que NÃO entra) no catálogo". Não envolva uma mensagem de erro nova em `__()` só porque ela aparece na tela.
...
- Hooks genéricos que já disparam automaticamente para qualquer gateway via `PaymentProcessor` (não precisa disparar você mesmo): `paycryptome_before_payment`/`paycryptome_after_payment` (actions), `paycryptome_payment_amount`/`paycryptome_payment_data` (filters), `paycryptome_for_woocommerce_gateway_loaded` (action, disparada no construtor da classe abstrata). Ver tabela completa em `AGENTS.md` § "Public hooks".
- Se o gateway novo tocar as libs `bitwasp/*`, **passe pelo `BitcoinAddressService`** em vez de chamá-las direto: elas emitem `E_DEPRECATED` no meio da requisição (o que já quebrou o redirect pós-save das settings On-Chain), e a máscara escopada vive lá — ver `suppress_vendor_deprecations()` e [DONE-CRYPTO-DEPRECATION-CONTINGENCY.md](./archive/DONE-CRYPTO-DEPRECATION-CONTINGENCY.md) (arquivado/gitignored, pode estar ausente no seu checkout). Nunca capture `\Throwable` na validação: extensão ausente é `\Error` e tem de propagar.
- Dois guards da suíte alcançam arquivo novo automaticamente, então não há passo a fazer — só não os contorne: `VendorReplaceGuardTest` varre **todo** PHP enviado (`includes/`, `exceptions/`, `templates/`, entrypoint, `uninstall.php`) proibindo `lastguest\Murmur`/`murmur3`/`BloomFilter` (pacote removido via `replace` — ver [DONE-LEAN-VENDOR-TREE.md](./archive/DONE-LEAN-VENDOR-TREE.md), arquivado/gitignored, pode estar ausente no seu checkout), e `PhpFloorConsistencyTest` fixa o piso de PHP nos quatro lugares que o declaram, caso o gateway novo precise subir esse piso.
- Se o novo gateway for **assíncrono** (como o Lightning, que resolve o invoice depois) e precisar de action de mudança de status para consumidores externos reagirem, siga o precedente `paycryptome_lightning_status_changed` — notificação pós-escrita disparada dentro do service de persistência somente quando um compare-and-swap realmente aplica a transição (ver `PayCryptoMeLightningDBStatementsService::transition_status()`). Ela não é entrega durável nem substitui idempotência no consumidor.
